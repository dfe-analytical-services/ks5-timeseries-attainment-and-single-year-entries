...
server <- function(input, output, session) {
  ...
  output$headBox1 <- renderUI({
    ...
    lapply(seq_along(latest), function(i) {
      fluidRow(
        valueBox(
          value = grade[i], subtitle = HTML(paste0("A level average result equivalent to ", strong(latest[i], "points"), ":", br(), input$headlineAps[i])), width = 12,
          color = "blue"
        )
      )
    })
  ...
  output$headBox2 <- renderUI({
    ...
    lapply(seq_along(latest), function(i) {
      fluidRow(
        valueBox(
          value = grade[i], subtitle = HTML(paste0("Applied general average result equivalent to ", strong(latest[i], "points"), ":", br(), input$headlineAps[i])), width = 12,
          color = "blue"
        )
      )
    })
  ...
  output$headBox3 <- renderUI({
    ...
    lapply(seq_along(grade), function(i) {
      fluidRow(
        valueBox(
          value = grade[i], subtitle = HTML(paste0("Tech level average result equivalent to ", strong(latest[i], "points"), ":", br(), input$headlineAps[i])), width = 12,
          color = "blue"
        )
      )
    })
...
")
  ...
  output$textSubAll <- renderText({
    ...
    paste(
      "The line chart shows the A level exam entries from ", val1, "to ", val2,
