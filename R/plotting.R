...
createApsTimeSeries <- function(dfAps, instGroup, instType, allGender) {
  ...
  fig1 <- ggplot(dfAps, aes(
    x = year_2013_2015, y = aps_2013_2015,
    color = school_type
  )) +
  ...
  fig2 <- ggplot(dfAps, aes(x = year_2016_2022, y = aps_2016_2022, color = school_type)) +
    geom_line(stat = "identity", size = 1.5) +
    geom_curve(aes(x = 2016.5, y = 50, xend = 2016, yend = 45),
      curvature = .3,
      arrow = arrow(length = unit(0.03, "npc"), type = "closed"),
      color = "black", size = .05, angle = 45
    ) +
    geom_curve(aes(x = 2020.5, y = 59, xend = 2021, yend = 52),
      curvature = -.3,
      arrow = arrow(length = unit(0.03, "npc"), type = "closed"),
      color = "black", size = .05, angle = 90
    ) +
    geom_curve(aes(x = 2019.5, y = 55, xend = 2020, yend = 50),
      curvature = -.3,
      arrow = arrow(length = unit(0.03, "npc"), type = "closed"),
      color = "black", size = .05, angle = 90
    ) +
...
createApsFmTimeSeries <- function(dfAps, instGroup, instType, fmGender) {
  ...
  fmFig <- ggplot(dfAps, aes(
    x = year_2016_2022, y = aps_2016_2022,
...
createTimeSeriesResult <- function(dfResult, resAll, subAll, subName) {
  ...
  ggplotly(fig) %>%
    layout(
      legend = list(orientation = "v", x = .99, y = .5),
      hovermode = "x, unified"
    )
