...
createTimeSeriesHeadline <- function(dfAps, allAps) {
  ...
  fig <- ggplot(
    fig,
    aes(x = Year, y = APS, group = Qualification, color = Qualification)
  ) +
...
createApsTimeSeries <- function(dfAps, instGroup, instType, allSex) {
  validate(need(
    dfAps$establishment_type,
    message = "To view charts select type of students and up to 4 institution types from the drop-down menus at the top page"
  ))
  ...
  fig1 <- ggplot(
    dfAps,
    aes(x = year_2013_2015, y = aps_2013_2015, color = establishment_type)
  ) +
    geom_line(stat = "identity", linewidth = 1.5) +
    geom_curve(
      aes(x = 2015.5, y = 50, xend = 2015, yend = 45),
    ...
    geom_label(
      aes(label = aps_grade_2013_2015, y = aps_2013_2015),
      show.legend = F
    ) +
    # ggtitle("\nAPS & grade\n2012/13 to 2014/15\n") +
  ...
  fig2 <- ggplot(
    fig2,
    aes(x = year, y = aps_per_entry, color = establishment_type)
  ) +
    geom_line(stat = "identity", linewidth = 1.5) +
    geom_curve(
      aes(x = 2016.5, y = 48, xend = 2016, yend = 45),
      curvature = .3,
      arrow = arrow(length = unit(0.03, "npc"), type = "closed"),
      color = "black",
      size = .05,
    ...
    geom_curve(
      aes(x = 2019.5, y = 54, xend = 2020, yend = 50),
      curvature = -.3,
      arrow = arrow(length = unit(0.03, "npc"), type = "closed"),
      color = "black",
      size = .05,
    ...
    geom_curve(
      aes(x = 2020.5, y = 58, xend = 2021, yend = 52),
      curvature = -.3,
      arrow = arrow(length = unit(0.03, "npc"), type = "closed"),
      color = "black",
      size = .05,
    ...
    geom_label(
      aes(label = aps_per_entry_grade, y = aps_per_entry),
      show.legend = F
    ) +
    ...
    annotate(
      ...
      x = 2016.5,
      y = 45,
      label = "Point scale changed in 2015/16 \nbut average grade remains consistent",
      color = "black",
      size = 4,
      vjust = -.3,
      hjust = 0
...
createApsFmTimeSeries <- function(dfApsFm, instGroup, instType, fmSex) {
  validate(need(
    dfApsFm$establishment_type,
    message = "To view chart select between 1 and 4 institution types from the drop-down menu at the top page."
  ))
  ...
  fmFig <- ggplot(
    ...
    geom_line(stat = "identity", linewidth = 1.5) +
    geom_curve(
      aes(x = 2019.5, y = 53, xend = 2020, yend = 50),
      curvature = -.4,
      arrow = arrow(length = unit(0.03, "npc"), type = "closed"),
      color = "black",
      size = .05,
    ...
    geom_curve(
      aes(x = 2020.5, y = 56, xend = 2021, yend = 53),
      curvature = -.4,
      arrow = arrow(length = unit(0.03, "npc"), type = "closed"),
      color = "black",
      size = .05,
...
createSexGap <- function(dfApsGap, instGroup, instType) {
  ...
  fig <- dfApsGap %>%
    rename(
      Year = "year",
      Sex_gap = "sex_gap",
      Institution_type = "establishment_type"
    )
...
createTimeSeriesSubject <- function(dfSubjectA, subAll, subName) {
  validate(need(
    dfSubjectA$subject_name,
    message = " To view chart select type of students and up to 4 subjects from the drop-down menus. Finally select a start year."
  ))
...
createTimeSeriesResult <- function(dfSubjectA, subAll, resAll, subName) {
  validate(need(
    dfSubjectA$subject_name,
    message = "To view chart select type of students, select up to 4 subjects and start year from the drop-down menus at the top of the page.  Finally select cumulative grade"
  ))
...
createTimeSeriesSubjectFm <- function(dfSubjectG, subByFm) {
  validate(need(
    dfSubjectG$subject_name,
    message = "To view chart select one subject and a start year from the drop-down menus at the top of the page."
  ))
...
createTimeSeriesResultFm <- function(dfSubjectG, subByFm, resByFm) {
  validate(need(
    dfSubjectG$subject_name,
    message = " To view chart select one subject and a start year from the drop-down menus and finally select cumulative grade"
  ))
  fig <- dfSubjectG %>%
    rename(
      Year = "year",
